...
export const LeadsTable = forwardRef<LeadsTableHandle, LeadsTableExternalProps>(
  function LeadsTable(
    ...
    const currentLimit =
      viewMode === "kanban" ? kanbanLimit || 100 : listLimit || 10;
    const setCurrentLimit =
      viewMode === "kanban" ? setKanbanLimit : setListLimit;
    ...
    const handleSelectAllCurrentPage = useCallback(() => {
      ...
      allIds.forEach((id) => {
        if (allSelected) {
          if (selectedIds.has(id)) handleSelectRow(id);
        } else {
          if (!selectedIds.has(id)) handleSelectRow(id);
        }
      });
    ...
    return (
      <div className="flex flex-col gap-4 w-full">
        <LeadsFilters
          ...
          uniqueSources={sourceFilters}
          uniquePriority={uniquePriorities}
          limit={currentLimit}
          onLimitChange={(val) => {
            setCurrentLimit?.(val);
            setCurrentPage(1);
