...
export function ActivityLogRow({ log }: ActivityLogRowProps) {
  ...
  const userAvatar = log.userId ? log?.user?.avatar : null;
    ...
    <tr className="border-t border-[#E8E8E9] bg-white hover:bg-[#F8FAFB] transition-colors">
      ...
      <td className="py-3 px-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[#1B1B1B] font-inter text-sm font-medium whitespace-nowrap">
            {userName}
          </span>
        </div>
      </td>
      ...
      <td className="py-3 px-4">
        <div className="flex flex-col gap-1">
          ...
          <div className="flex items-center gap-2">
            <span
              className={`text-xs font-medium capitalize ${actionColors[log.action] || "text-[#777980]"}`}
            >
              {log.action}
            </span>
            <span className="text-[#777980] text-xs">•</span>
            <span
              className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${entityColors[log.entity] || "bg-[#F8FAFB] text-[#777980]"}`}
            >
              {log.entity}
            </span>
          </div>
